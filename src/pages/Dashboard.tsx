...
export default function Dashboard() {
  ...
  return (
    <Layout title="经智AI智能体工作平台" subtitle="Economic Crime Intelligence Platform" showBack={false}>
      <Reveal
        as="section"
        className="mb-12 grid gap-10 rounded-[2.5rem] border border-slate-200/80 bg-white p-10 text-slate-900 shadow-[0_60px_140px_rgba(15,23,42,0.12)] lg:grid-cols-[1.35fr,1fr]"
        id="overview"
      >
        <div className="flex flex-col justify-between gap-10">
          <div className="space-y-6">
            <span className="inline-flex items-center gap-2 rounded-full border border-slate-200/80 bg-white px-4 py-1 text-xs font-semibold uppercase tracking-[0.3em] text-slate-500 shadow-sm">
            ...
            {supportModules.map((module, index) => {
              ...
              return (
                <Reveal key={module.title} delay={0.08 * (index + 1)} className="h-full">
                  <Link
                    to={module.link}
                    className="group relative flex h-full flex-col gap-3 overflow-hidden rounded-3xl border border-slate-200/80 bg-white p-5 text-slate-700 shadow-[0_36px_90px_rgba(15,23,42,0.08)] transition hover:-translate-y-1 hover:shadow-[0_42px_120px_rgba(15,23,42,0.12)] hover:text-police-blue"
                  >
                    <span
                    ...
                    <div className="relative flex items-center gap-3">
                      <span className={`${iconBaseClass} ${module.accent}`}>
                    ...
                    <div className="relative mt-auto flex flex-wrap gap-2 text-[0.68rem] uppercase tracking-[0.2em] text-slate-400">
                      {module.metrics.map((item) => (
                        <span key={item} className="rounded-full border border-slate-200/70 bg-white px-3 py-1">
                          {item}
                        </span>
                      ))}
                    </div>
          ...
          <div className="flex flex-wrap gap-4 text-sm">
            <Link
              to="/stats"
              className="group inline-flex items-center gap-2 rounded-2xl border border-slate-200/80 bg-white px-5 py-3 font-semibold text-police-blue shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
            >
              运行态势总览
              <ArrowUpRight className="h-4 w-4 transition-transform duration-300 group-hover:-translate-y-0.5 group-hover:translate-x-1" />
            </Link>
        ...
        {primaryModule && PrimaryIcon && (
          <Reveal delay={0.05} className="h-full">
            <Link
              ...
              className="group relative flex h-full flex-col gap-4 overflow-hidden rounded-[2.2rem] border border-slate-200/80 bg-white p-8 text-slate-700 shadow-[0_42px_120px_rgba(15,23,42,0.1)] transition-transform duration-500 hover:-translate-y-1 hover:shadow-[0_48px_150px_rgba(15,23,42,0.14)] hover:text-police-blue"
            >
              ...
              <div className="relative flex items-center gap-4">
                <span className={`${iconBaseClass} ${primaryModule.accent ?? 'icon-holo--blue'}`}>
              ...
              <div className="relative grid gap-3 text-sm text-slate-600 sm:grid-cols-2">
                {primaryModule.metrics.map((item) => (
                  <span key={item} className="flex items-center gap-2 rounded-2xl border border-slate-200/70 bg-white px-4 py-2">
      ...
      <Reveal as="section" className="mb-14">
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3" id="cases">
          {caseCategories.map((category, index) => {
            ...
            return (
              <Reveal key={category.title} delay={0.05 * index} className="h-full">
                <Link
                  to={category.link}
                  className="group flex h-full flex-col gap-4 rounded-3xl border border-slate-200/80 bg-white p-6 text-slate-700 shadow-[0_36px_90px_rgba(15,23,42,0.08)] transition hover:-translate-y-1 hover:shadow-[0_42px_120px_rgba(15,23,42,0.12)] hover:text-police-blue"
                >
                  <div className="flex items-center gap-4">
                    <span className={`${iconBaseClass} ${category.accent}`}>
      ...
      <Reveal as="section" className="mb-14 grid gap-6 lg:grid-cols-[1.1fr,1fr]" id="documents">
        <div className="rounded-[2rem] border border-slate-200/80 bg-white p-8 text-slate-900 shadow-[0_48px_120px_rgba(15,23,42,0.1)]">
            ...
            {documentModules.map((module, index) => {
              ...
              return (
                <Reveal key={module.title} delay={0.08 * index} className="h-full">
                  <Link
                    to={module.link}
                    className="group flex h-full flex-col gap-3 rounded-2xl border border-slate-200/80 bg-white p-6 text-slate-600 shadow-[0_32px_80px_rgba(15,23,42,0.08)] transition hover:-translate-y-1 hover:shadow-[0_38px_110px_rgba(15,23,42,0.12)] hover:text-police-blue"
                  >
                    <span className={`${iconBaseClass} ${module.accent}`}>
                      <Icon className="h-6 w-6" />
                    </span>
        ...
        <div className="flex flex-col gap-5">
          <Reveal className="rounded-[2rem] border border-slate-200/80 bg-white p-6 text-slate-900 shadow-[0_36px_90px_rgba(15,23,42,0.1)]" delay={0.1}>
            <h4 className="text-lg font-semibold">实时通知</h4>
            <p className="mt-2 text-sm text-slate-500">智能体更新、模型迭代与运维公告统一呈现。</p>
            <ul className="mt-6 space-y-4 text-sm text-slate-600">
              <li className="rounded-2xl border border-slate-200/70 bg-white p-4 shadow-sm">
          ...
          <Reveal delay={0.18} className="h-full">
            <Link
              to="/stats"
              className="flex h-full flex-col justify-between rounded-[2rem] border border-slate-200/80 bg-white p-6 text-slate-700 shadow-[0_36px_90px_rgba(15,23,42,0.08)] transition hover:-translate-y-1 hover:shadow-[0_42px_120px_rgba(15,23,42,0.12)] hover:text-police-blue"
            >
              <div>
      ...
      <Reveal as="section" className="mb-10" id="exclusive">
            ...
            return (
              <Reveal key={item.title} delay={0.08 * index} className="h-full">
                <Link
                  ...
                  className="group flex h-full flex-col gap-4 rounded-3xl border border-slate-200/80 bg-white p-6 text-slate-700 shadow-[0_36px_90px_rgba(15,23,42,0.08)] transition hover:-translate-y-1 hover:shadow-[0_42px_120px_rgba(15,23,42,0.12)] hover:text-police-blue"
                >
                  <span className={`${iconBaseClass} ${item.accent}`}>
                    <Icon className="h-6 w-6" />
                  </span>
                  <div>

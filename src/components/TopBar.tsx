...
export default function TopBar({ title, subtitle, showBack = true, onMenuClick }: TopBarProps) {
  ...
  const navItems = [
    { label: '智能体中枢', href: '/home#overview', icon: Sparkles },
    { label: '案件矩阵', href: '/home#cases', icon: Shield },
    { label: '文书服务', href: '/home#documents', icon: FileText },
    { label: '运行态势', href: '/stats', icon: BarChart3 },
    { label: '环境支撑', href: '/support/chrome-installer', icon: Compass }
  ];

  return (
    <header className="fixed inset-x-0 top-0 z-40 border-b border-slate-200/70 bg-white/90 shadow-[0_18px_55px_rgba(148,163,184,0.22)] backdrop-blur-xl">
      <div className="mx-auto flex w-full max-w-7xl items-center gap-4 px-4 py-4 text-slate-900 sm:px-6 lg:gap-6 lg:px-10">
        <div className="flex flex-1 items-center gap-3 sm:gap-4">
          ...
          <div className="relative flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-600 via-indigo-500 to-sky-500 text-white shadow-[0_18px_38px_rgba(59,130,246,0.35)]">
        ...
        <nav className="hidden flex-1 items-center justify-center gap-2 rounded-full border border-slate-200/80 bg-white/95 px-1.5 py-1 shadow-inner sm:flex">
          {navItems.map(({ label, href, icon: Icon }) => (
            <Link
              key={label}
              to={href}
              className="group inline-flex items-center gap-2 rounded-full px-4 py-2 text-[0.75rem] font-semibold uppercase tracking-[0.32em] text-slate-500 transition hover:-translate-y-0.5 hover:bg-slate-100/70 hover:text-police-blue"
            >
              <Icon className="h-4 w-4 text-slate-400 transition group-hover:text-police-blue" />
              {label}
            </Link>
          ))}
        </nav>
        <div className="flex flex-1 items-center justify-end gap-3">
          ...
          <div className="hidden items-center gap-2 rounded-full border border-slate-200/80 bg-white/90 px-2 py-2 shadow-[0_12px_28px_rgba(148,163,184,0.22)] sm:flex">
            <Link
              to="/stats"
              className="inline-flex items-center gap-2 rounded-full border border-slate-200/80 bg-white px-4 py-2 text-[0.7rem] font-semibold uppercase tracking-[0.3em] text-slate-600 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md hover:text-police-blue"
            >
